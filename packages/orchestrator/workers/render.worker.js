const { resolveRenderAssembly, getRenderQueueError, isSlideshowProject } = require("@cosyl/shared");
const { randomUUID } = require("node:crypto");
const { PROJECT_STATUSES, RENDER_JOB_STATUSES } = require("@cosyl/shared/types/production");
...
async function processRenderJob(job, { jobsRepository, projectsRepository, composeVideo }) {
  ...
  try {
    ...
    const nextAssembly = resolveRenderAssembly(project);
    ...
    // Composite the real MP4 when a compositor is wired in (local driver).
    // Falls back to the metadata-only package if compositing is unavailable.
    let composed = null;
    let storagePath = exportedPackage.storagePath;
    if (typeof composeVideo === "function") {
      try {
        composed = await composeVideo({ project, assembly: nextAssembly });
        storagePath = composed.storagePath;
        activeJob = await updateJob(jobsRepository, activeJob, {
          output: appendEvent(activeJob.output, {
            step: "export_video",
            message: `Composited ${composed.sceneCount} scenes into ${composed.fileName} (${composed.resolution}, ${composed.hasAudio ? "with audio" : "silent"}).`,
          }),
        });
      } catch (composeError) {
        // Surface the failure — a personal tool should fail loudly, not silently ship a stub.
        throw new Error(
          `Final composition failed: ${composeError instanceof Error ? composeError.message : composeError}`
        );
      }
    }
    ...
    const output = {
      ...
      composed,
      finalAsset: {
        fileName: composed?.fileName || nextAssembly.output.fileName,
        storagePath,
        resolution: composed?.resolution || nextAssembly.resolution,
        format: nextAssembly.output.format,
        aspectRatio: nextAssembly.aspectRatio,
        sizeBytes: composed?.sizeBytes ?? null,
        durationSeconds: composed?.durationSeconds ?? null,
        hasAudio: composed?.hasAudio ?? null,
