...
const SCHEMAS_ROOT = path.resolve(__dirname, "../factory/schemas");
...
function buildImagePromptHandoff({ scene, project, variant }) {
  ...
  const handoff = {
    ...
    continuity_indices: {
      lighting: sanitizeSummaryText(
        `${variant.mood || "balanced"} lighting with a ${variant.palette || "neutral"} accent`,
        "consistent cinematic lighting"
      ),
      environment_anchor_objects: sanitizeSummaryText(
        referenceAnchors.length > 0 ? referenceAnchors.join(", ") : scene.visualIntent,
        "stable environment and anchor objects"
      ),
      core_subject: sanitizeSummaryText(
        project.title || project.goal || scene.narration,
        "primary scene subject"
      ),
    },
  };
...
function buildImageGenerationHandoff({ scene, project, variant }) {
  const handoff = {
    ...
    framing_protection: sanitizeSummaryText(
      `Maintain the ${variant.shot || "intended"} framing with readable composition and a clean horizon/edge line`,
      "Maintain the intended framing and composition readability"
    ),
    staging_protection: sanitizeSummaryText(
      `Keep the core subject staged consistently for a ${variant.mood || "balanced"} ${variant.shot || "shot"}`,
      "Keep subject staging stable and uncluttered"
    ),
    animation_readiness: sanitizeSummaryText(
      "Clean silhouette and layered depth so the still can be animated with restrained motion downstream",
      "Composition supports downstream motion"
    ),
    failure_risk_notes: sanitizeSummaryText(
      "Risk of anatomy distortion, identity drift or a cluttered background; keep the subject clean and readable",
      "Watch for anatomy distortion and identity drift"
    ),
  };
...
function buildMotionVideoHandoff({ scene, project, variant, approvedImage }) {
  ...
  const handoff = {
    ...
    source_context: {
      ...
      continuity_indices: {
        lighting: sanitizeSummaryText(
          `${approvedImage?.mood || variant.energy || "balanced"} lighting with a ${approvedImage?.palette || "neutral"} accent`,
          "consistent cinematic lighting"
        ),
        environment_anchor_objects: sanitizeSummaryText(
          scene.visualIntent,
...
function buildVoiceDirectionHandoff({ project, audio }) {
  ...
  const continuityConstraints = uniqueStrings([
    ...
    "deliver the narration as a dry voice stem without music or SFX baked in",
...
function buildSoundtrackDirectionHandoff({ project, audio }) {
  ...
  const handoff = {
    ...
    music_plan: {
      ...
      generation_brief: sanitizeSummaryText(
        typeof music.generationBrief === "string" ? music.generationBrief : "",
        "Build a dedicated soundtrack stem that follows the script arc, the project DNA and the configured tone."
      ),
    ...
    sfx_plan: {
      ...
      cue_focus: uniqueStrings([
        typeof sfx.designBrief === "string" ? sfx.designBrief : null,
        ...cueFocus,
      ]).slice(0, 6),
    },
  };
