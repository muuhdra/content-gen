...
const { RENDER_JOB_STATUSES } = require("../../libs/types/production");
...
function createLocalOrchestratorQueue({ jobsRepository, projectsRepository }) {
  return {
    driver: "local",
    async enqueueRenderJob(projectId, payload = {}) {
      ...
      queueMicrotask(() => {
        void processRenderJob(job, { jobsRepository, projectsRepository });
      });
...
function createBullMqOrchestratorQueue({ jobsRepository, projectsRepository, redisUrl }) {
  ...
  new bullmq.Worker(
    ...
    async (bullJob) => {
      ...
      await processRenderJob(job, { jobsRepository, projectsRepository });
    },
    { connection }
  );
...
function createOrchestratorQueue({ jobsRepository, projectsRepository }) {
  ...
  return createLocalOrchestratorQueue({ jobsRepository, projectsRepository });
}
